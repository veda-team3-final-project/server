...
이동 중인 차량이 있다면, 차량의 이동 벡터와 인간의 이동 벡터를 외적하여 인간의 좌측에 있는 차량과 우측에 있는 차량을 구분합니다.
각 방향에 있는 차량들의 벡터와 인간의 이동 벡터를 내적하여 차량이 인간을 향해 접근하고 있는지 판단합니다.
차량이 인간을 향해 접근하고 있다면, 경고신호를 띄우고 동시에 ffmpeg를 통해 RTSP 스트림에서 이미지를 캡처하고, SQLite DB에 저장합니다.
캡처된 이미지는 메타데이터의 타임스탬프(UTC)를 KTC로 변환한 것을 파일명으로 하여 저장됩니다. 
...
bool is_linecrossing_event(const string& block, string& object_id, string& rule_name) {
    //cout << "[DEBUG] 이벤트 블럭 수신 (길이: " << block.length() << ")" << endl;
    ...
    bool name_ok = regex_search(block, match, name_regex);
...
bool is_any_vehicle_moving(const string& block, const string& rule_name, string& direction_info) {
    lock_guard<mutex> lock(data_mutex);

    bool frame_logged = false;

    // 현재 프레임에 감지된 인간과 차량의 중심 좌표를 저장하기 때문에 함수 내에 선언
    unordered_map<int, Point> current_human_centers;  // 현재 프레임에 감지된 인간들
    unordered_map<int, Point> current_vehicle_centers;  // 현재 프레임에 감지된 차량들
    ...
    auto human_begin = sregex_iterator(block.begin(), block.end(), human_regex);
    for (auto it = human_begin; it != sregex_iterator(); ++it) {
        ...
        // 새로 들어온 사람이 아니라면 이전 위치와 비교하여 업데이트 여부 판단
    ...
    // 기록에 남아있던 사람 ID가, 이번 프레임에는 감지되지 않았다면 사라진걸로 간주하고 출력
    for (const auto& [prev_id, _] : human_history) {
        if (current_human_centers.find(prev_id) == current_human_centers.end()) {
            if (!frame_logged) {
                cout << "frame update!" << endl;
                frame_logged = true;
            }
            cout << "[Human] " << prev_id << " disappear" << endl;
        }
    }

    // [2] linecrossing 발생 사람의 벡터 구하기
    Point human_vec = {0, 0};
    Point human_center = {0, 0};
    bool human_ok = false;

    // rule_name이 비어있지 않은 경우(=linecrossing이 발생한 경우)에만 사람 벡터를 추정
    if (!rule_name.empty()) {
        regex id_regex("<tt:SimpleItem Name=\"ObjectId\" Value=\"(\\d+)\"");
        smatch match;
        if (regex_search(block, match, id_regex)) {
            // block에서 ObjectId 추출
            int human_id = stoi(match[1]);
            // id에 해당하는 사람의 위치 이력에서 벡터 추정
    ...
    if (!human_ok) return false; // 판단 불가 시 리턴

    // [3] 차량 추적 및 위험 판단
    bool risk_detected = false;
    // frame 블럭 내에 존재하는 모든 vehile 객체를 추적
    regex vehicle_regex("<tt:Object ObjectId=\"(\\d+)\">[\\s\\S]*?<tt:Type[^>]*?>Vehicle</tt:Type>[\\s\\S]*?<tt:CenterOfGravity x=\"([\\d.]+)\" y=\"([\\d.]+)\"");
    auto begin = sregex_iterator(block.begin(), block.end(), vehicle_regex);

    // 현재 프레임에 감지된 차량들의 위치를 업데이트 및 조건 계산
    for (auto it = begin; it != sregex_iterator(); ++it) {
        int vehicle_id = stoi((*it)[1]);
        Point cog = {stof((*it)[2]), stof((*it)[3])};

        bool is_new = trajectory_history.find(vehicle_id) == trajectory_his.tory.end();
        ...
        if (trajectory_history[vehicle_id].size() >= 2) {
...
void metadata_thread() {
    ...
    while (fgets(buffer, BUFFER_SIZE, pipe)) {
        ...
        if (xml_buffer.find("</tt:MetadataStream>") != string::npos) {
            //<tt:MetadataStream> ~ </tt:MetadataStream> 사이의 블럭을 추출
            string block = xml_buffer;
            xml_buffer.clear();


            /*
            block을 event 블럭, videoanalytics 블럭으로 구분하여
            currnet_event_block, current_frame_block에 갱신하여 저장
            event 블럭은 라인크로싱 이벤트가 발생했을 때만 갱신하고,
            videoanalytics 블럭은 항상 갱신하여 저장합니다.
            event 블럭의 라인크로싱 이벤트가 갱신되었을 때 is_human(current_frame_block, object_id)을 사용하여
            event 블럭의 object_id가 인간인지 확인합니다.

            is_any_vehicle_moving(current_event_block, current_frame_block, rule_name, direction_info) 
            위와 같이 수정하여 로직 동작하도록 합니다.
            */
            ...
            if (is_linecrossing_event(block, object_id, rule_name)) {
                ...
                // VideoAnalytics 프레임 블럭인지 확인
                // ***********************************************************************************************************
                // ??? 블럭이 라인크로싱 블럭이면 그 안에 VideoAnalytics가 안들어있지 않나? 아니 여태까지 어떻게 이게 true가 된거지????
